...
验证通过持续注入**全随机 TCP/UDP 噪声包**，是否可以有效破坏中间人对流量模式的机器学习分析，防止攻击者从加密流量中推断用户的上网意图。

## 核心原理

```
应用/浏览器 → 系统代理 (SOCKS5 127.0.0.1:1086) → NoiseTunnel
                                                    │
                                          ┌─────────▼─────────┐
                                          │  真实流量 → 原样转发 │
                                          │                    │
...
| 平台 | 设置方式 |
|------|---------|
| **Windows** | 设置 → 网络和 Internet → 代理 → 使用 SOCKS5 代理 → `127.0.0.1:1086` |
| **Linux** | `export ALL_PROXY=socks5://127.0.0.1:1086` |
| **macOS** | 系统偏好设置 → 网络 → 高级 → 代理 → SOCKS5 代理 → `127.0.0.1:1086` |
| **浏览器** | SwitchyOmega 等插件直接配置 SOCKS5 代理 |
