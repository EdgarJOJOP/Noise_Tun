...
- **TCP 噪声**：生成结构完整的 IP + TCP + TLS Client Hello 数据包，**每个字节均为 CSPRNG 全随机**
- **UDP 噪声**：生成 IP + UDP + 随机载荷数据包，全部字段 CSPRNG 随机
- **目标发现**：通过 DNS-over-HTTPS (DoH) 加密解析随机域名，不泄露 DNS 查询意图
- **自适应密度**：根据真实流量速率动态调整噪声注入密度
- **无 TUN 虚拟网卡**：纯 SOCKS5 代理，用户态运行，需管理员权限
- **配合全局代理即可代理全部真实流量走localhost:1086**：
    [v2rayN](https://github.com/2dust/v2rayN)或[v2rayNG](https://github.com/2dust/v2rayNG)添加SOCKS配置后选择设置为活动，再点击选择软件下方 自动配置系统代理 和 V4-全局 就可轻松使用该全随机噪声。
...
```bash
conda create -n [名字] python==3.13.2
...
| 低流量 | 1-10 连接/10秒 | 50% | 将处于高流量收集到的域名用来模拟低流量时期 |
